...
async function updateOfficial(dateStr = '2026-08-25') {
  const srcDir = path.join(process.cwd(), 'data', 'images', 'perfect_test2');
  ...
  const files = [
    { src: 'briefing_full.png', dst: `farol_briefing_${dateStr}.png`, type: 'farol-briefing', caption: 'Briefing de Mercado — Farol do Mercado' },
    { src: 'gps_full.png', dst: `farol_gps_${dateStr}.png`, type: 'farol-gps', caption: 'GPS de Mercado — Farol do Mercado' },
    { src: 'radar_full.png', dst: `farol_radar_${dateStr}.png`, type: 'farol-radar', caption: 'Radar de Mercado & Tickers — Farol do Mercado' },
  ];
  ...
  if (day) {
    ...
    console.log('✓ Banco SQLite atualizado com os prints completos!');
  }
}

updateOfficial().catch(console.error);
